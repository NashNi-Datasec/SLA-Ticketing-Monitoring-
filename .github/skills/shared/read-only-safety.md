...
- `.github/workflows/dpsa-pr-comment.yml` — **read only** (VS Code `/dpsa` picks the same PR base as the Action)
...
(`git rev-parse --abbrev-ref HEAD` is read-only — Jira keys in the branch name. `git rev-parse --verify` and `git branch -r` are read-only — pick the VS Code diff base.)
