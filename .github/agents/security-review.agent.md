...
- Follow `.github/skills/dpsa/SKILL.md` for the report. Instruction Scenarios 1–2 belong to the automatic instructions, not this agent.
...
2. Same base as `/dpsa` (workflow file / staging-UAT, not a blind `main` default)
3. Staged changes (`git diff --cached`)
4. Unstaged changes

If no diff exists after trying the above, emit **REVIEW INCOMPLETE** — do not ask a free-form question outside the template.
...
- Match the `/dpsa` skill template for this run (VS Code preview vs GitHub Actions PR Scenario 1 / 2 / 3 / 4)
- Include **Change summary**
- Include the skill’s Result line (Not Required, PASS, Security Issue/s identified, or REVIEW INCOMPLETE)
- Include **What to do next** when the skill template has that section
- Each finding: severity, file, line, impact, recommendation, steps to reproduce, confidence, **OWASP Category**, OWASP calculator URL, severity rationale
- If no diff / cannot analyze: **REVIEW INCOMPLETE** — not a free-form question outside the template
...
All security review output must still be reviewed by a human before merge. **Critical** still needs a fix or `dpsa-exception` plus a PR comment with the Finding Analyst result. High / Medium / Low may merge after Finding Analyst triage.
