...
| **GitHub Action** | Same-repo PR into staging/UAT | Official DPSA comment (PR 1–4); Critical (PR 4) can fail the check |
...
Instructions **prevent** issues while coding. Agents provide **selectable personas**. Skills **detect** issues on demand. `/dpsa` and Security Review are two entry points to the same review; the **PR comment** is the short official template.
